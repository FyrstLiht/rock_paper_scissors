...
const animTimings = [10, 1500, 2500];
let playerWins = 0,
    cpuWins = 0,
    winner = '',
    loser = '',
...
function getResult(event) {
    ...
    cpuSelection = choices[Math.floor(Math.random() * 3)];
...
function animateHands(handID, array) {
    ...
    let i = 0;
    if (winner === 'draw') {
        array[1] = `${handID.replace("Hand", "")}Draw`;
...
function showRPS() {
    const hands = document.querySelectorAll('.hands');
    hands[0].textContent = `${playerSelection}`;
    hands[1].textContent = `${cpuSelection}`;
